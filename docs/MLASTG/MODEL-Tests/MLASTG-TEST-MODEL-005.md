...
## Control Reference
**Controls Tested:** MLASVS-MODEL-007 (Model Versioning), MLASVS-MODEL-008 (Model Signing), MLASVS-MODEL-014 (Secure Model Serialization), MLASVS-MODEL-015 (Model Rollback Capability), MLASVS-MODEL-030 (Model Provenance Attestation — L2)
...
2. Verify that each model version has:
   - Immutable version identifier (hash or sequential version)
   - Timestamp of creation
   - Training data hash or reference
   - Code commit hash used for training
   - Author/creator identity
3. Verify that model artifacts are never overwritten (append-only)
4. **Pass if:** Complete version history exists for all production models with required metadata
5. **Fail if:** Versions are missing, overwritten, or lack required metadata

### Step 2: Verify Model Signing
1. Check that model files are cryptographically signed:
   ```bash
   # Verify GPG signature
   gpg --verify model.bin.sig model.bin
   
   # Or verify Sigstore/Cosign signature
   cosign verify --key cosign.pub model:latest
   ```
2. Attempt to deploy an unsigned model and verify it is rejected
3. **Pass if:** All production models are signed; unsigned models are rejected at deployment
4. **Fail if:** Models can be deployed without signing, or signature verification is not enforced
...
1. Review model loading code for hash verification:
   ...
   ```
2. Tamper with a model file (add a null byte) and verify detection
3. **Pass if:** Hash verification is enforced at load time; tampered files are rejected
4. **Fail if:** Models can be loaded without integrity verification

### Step 4: Verify Safe Serialization
1. Scan all model files for unsafe serialization patterns:
   ```bash
...
4. **Pass if:** No unsafe serialization detected; SafeTensors used where possible
5. **Fail if:** Pickle files with `__reduce__` patterns are found, or SafeTensors is not used for new models
...
4. **Pass if:** Rollback completes within the documented Recovery Time Objective (RTO)
5. **Fail if:** Rollback fails, is not documented, or exceeds RTO

### Step 6: Verify Model Provenance Attestation (L2)
1. Verify that each production model has a provenance attestation document including:
   - Model origin (trained from scratch, fine-tuned, or downloaded)
   - Training data provenance chain (source, hash, transformations)
   - Training environment (compute, software versions)
   - Training process (hyperparameters, random seeds)
   - Cryptographic chain of custody from training to deployment
2. **Pass if:** Complete provenance attestation exists for all production models
3. **Fail if:** Any production model lacks provenance documentation
...
| L2 | All L1 controls met; model provenance attestation documented; complete chain of custody |
...
## References
- **MITRE ATLAS:**
  - `AML.TA0006` — Persistence (model registry manipulation)
  - `AML.TA0002` — Initial Access (supply chain)
- **MLASWE:** MLASWE-0009 (Supply Chain Compromise)
- **NIST AI RMF:** MAP 1.5 (Risk identification), GOVERN 1.6 (Supply chain risk)
- **Related Standard:** Sigstore (model signing), SLSA (Supply chain Levels for Software Artifacts)
