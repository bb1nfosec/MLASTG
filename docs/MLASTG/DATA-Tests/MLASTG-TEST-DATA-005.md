...
3. **Pass if:** Complete FL architecture documentation exists with clearly defined trust boundaries.
4. **Fail if:** The architecture is undocumented or trust boundaries between server and participants are undefined.
...
3. Measure the impact on global model accuracy:
   ...
   ```
4. **Pass if:** Global model accuracy degrades by < 10% when up to 30% of participants are Byzantine.
5. **Fail if:** A single malicious participant or a small minority can degrade global model accuracy by > 20%.
...
3. **Pass if:** Gradient inversion techniques fail to recover meaningful training data.
4. **Fail if:** Original training samples can be reconstructed with high fidelity from shared gradients.
...
3. **Pass if:** All contributions are appropriately attributed and identity spoofing is effectively prevented.
4. **Fail if:** Participants can submit anonymous updates or forge the identity of other participants.
...
3. **Pass if:** Individual updates cannot be reconstructed or inspected by the server prior to aggregation.
4. **Fail if:** The aggregation server has plaintext access to and can inspect individual participant gradients.
...
3. **Pass if:** All malformed, oversized, or replayed updates are correctly rejected.
4. **Fail if:** Malformed or invalid updates are accepted and aggregated into the global model.
